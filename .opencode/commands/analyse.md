...
6. Archive every root `.opencode/plans/*.md` file currently present in `.opencode/plans/` into `.opencode/plans/archive/v-<n>/<YYYYMMDD>T<HHMMSS>-<filename>`, regardless of content changes.
...
- Apply the archive-control flow (Step 2) on every root `.opencode/plans/*.md` file when archiving is enabled, and archive all files regardless of content changes.
...
Archive:     <list of files moved to .opencode/plans/archive/v-<N>/ or "skipped (archive=none)"
